...
An interactive document has *user inputs* and *display outputs*. The user sets the inputs as desired, the outputs respond accordingly. In writing an interactive documents, you have to specify four things:
...
In practice, you would suppress the display of the widget-creating command.  But this is a document about how to use Shiny, so you might as well see them.
...
make_my_plot <- reactive( 
  {
    ...
    ggplot(vals, aes(x = x)) + geom_histogram(binwidth = input$nbins)
  }
)
```
