...
* Using `mScatter()` [PUT THE LINK TO SLIDES HERE.  Include Exercises.Rmd: GraphicsQuickStart/One] 
    1. Define the frame (faceting also is part of the frame)
    2. map variables to the aesthetics
* Activity: Visualizing a data frame: Use `NHANES` from the exercise. [Note to DTK: need more scaffolding here (list of interesting datasets, how to see the codebooks, etc.)]
``` 
...
Simple to moderately complicated data wrangling tasks in the context of bike rentals in thw Washington, DC area. For quick browsing, here's the [HTML file](Handouts/BikeSharingBasics.html).  But work with the `.Rmd` as shown.

1. Download and edit the `.Rmd` template file. Copy these commands. 
```{r eval=FALSE}
download.file("http://dtkaplan.github.io/USCOTS-2015/Handouts/BikeSharingBasics.Rmd", #"http://tiny.cc/dcf/BikeSharingBasics.Rmd",
