...
2. Introduce your neighbors, and form teams for later. Two to three people per team, with nice mix of skills. Give your team a name so that we can refer to it. [15 min]
...
1. Some grammar [DTK, 15 min]
    ...
    * Glyph-ready data
#. Map variables to aesthetics with `mScatter()`, `mBar()`, `mDistribution()`, `mWorldMap()` [Note to DTK: why not use mplot here?]
    ```{r eval=FALSE}
...
* Other sources of data. Something easy they can do: NYC open data, CDC.  At the level of downloading CSV files. [DTK to find example from Data Computing book.]  Bike sharing? Restaurants?  Scaffold 70% for them and have them do one. [30 min work, 10 min presentation]
